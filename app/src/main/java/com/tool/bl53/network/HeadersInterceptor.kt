        ...
        if ("GET" == request.method) {
            ...
            if (request.body is FormBody) {
                ...
                for (i in 0 until newBody.size) {
                    LogUtils.e(TAG, "---" + newBody.name(i) + " = " + newBody.value(i))
                }
                requestBuilder
            ...
            if (request.body is MultipartBody) {
                LogUtils.e(TAG, "---请求头参数：MultipartBody")
                ...
                for (i in parts.indices) {
                    ...
                    if (body1.contentLength() < 100) {
                        ...
                        //打印 name和value
                        if (names.size > i) {
                            LogUtils.e(TAG, "---params = " + names[i] + " = " + value)
                        }
                    } else {
                        if (names.size > i) {
                            LogUtils.e(TAG, "---params = " + names[i])
                        }
                    }
                }
                requestBuilder.post(newBody)
